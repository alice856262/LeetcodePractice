...
# Trainers. 
# 
#  Related Topics Array Two Pointers Greedy Sorting 👍 4269 👎 397
...
class Solution:
    def findContentChildren(self, g: List[int], s: List[int]) -> int:
        ...
        child = 0  # Pointer/index for children (greed factors)
        cookie = 0  # Pointer/index for cookies (sizes)

        while child < len(g) and cookie < len(s):
            # If the current cookie satisfies the current child's greed
            if s[cookie] >= g[child]:
                child += 1  # Assign cookie to child and move to the next child
