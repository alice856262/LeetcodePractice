...
# O(1) memory?
# 
#  Related Topics Hash Table Linked List Two Pointers 👍 15420 👎 1398
...
class Solution:
    ...
    def getIntersectionNode(self, headA: ListNode, headB: ListNode) -> Optional[ListNode]:
        ...
        # Traverse both lists
        while pA != pB:
            # Move to the next node in listA, or switch to listB's head
            pA = pA.next if pA else headB
            # Move to the next node in listB, or switch to listA's head
            pB = pB.next if pB else headA

        # Either both pointers meet at the intersection node or at None
        return pA
